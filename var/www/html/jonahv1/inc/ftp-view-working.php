...
foreach(glob($voduploaddir.'*', GLOB_ONLYDIR) as $i)						// for each folder found in the 'voduploaddir' root ($i = '/home/sony' )
	...
	// echo ("<td class='td-ffmpeg'>debug array of ".$ffprobe."</td>"); 		// ffprobe																	
  	echo ("<td class='td-video underlined'>".$filevideo."</td>"); 												// ffmpeg output relevant to videofile 				// cleanup
	echo ("<td class='td-audio underlined'>".$testAudio."</td>"); 												// ffmpeg output relevant to videofile 				// cleanup
	...
	echo ("</tr>");
		...
		echo ("<tr><td><span class='countdown'>".$client."</span></td><td colspan='7'> ".$filefound." (not valid media type - ".$fileage." mins ago)</td><td><img src='/jonahv1/images/unknown.png' /></td></tr>");
			...
			{
				rename($filefound,$pathtoxml.$loc3);				// move xml to '/var/www/html/jonahv1/xml/'
			}
			else 
			{
				if ($fileage>$minstowait) 
				{
				rename($filefound,$orphanpath.$loc3);		// move non media, non-xml to '/var/www/html/jonahv1/orphans/'
				}
			} // end xml type
		}
	}
