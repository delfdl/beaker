...
foreach(glob($voduploaddir.'*', GLOB_ONLYDIR) as $i)						// for each folder found in the 'voduploaddir' root ($i = '/home/sony' )
	...
	foreach(glob($watchfolder.'*', GLOB_ONLYDIR) as $i)						// for each folder found in the 'voduploaddir' root ($i = '/home/sony' )
		{
			//echo ("i: ".$i." | ");													// add trailing slash to folder
    ...
    $videofilename=basename($filefound,'.'.$tempinfo['extension']);			// find filename without path and extension (e.g. 'Shakira Liar')
		$tempinspect_file=$pathtoinspect.$videofilename.".txt";					// full path path to 'ffmpeg -i' output (e.g. '/var/www/html/jonahv1/txtout/shakira liar.txt')
   	...
   	$ffmpeg_inspect="ffmpeg -i '".$filefound."' 2> ".$inspect_file;			// ffmpeg command (e.g. 'ffmpeg -i '/home/sony/Shakira Liar.mpg' '/var/www/html/jonahv1/txtout/sony-shakira_liar.txt')
  ...
  $fileinfo3 = stristr($fileinfo2,"Stream #0:1",True); 							//
	$filevideo = stristr($fileinfo3,"Stream #0:0");										//
	 ...
	 // check for non-existance of video codec	
	 if (!strpos($filevideo,'ideo'))
	 	{$filevideo='unable to identify';} 
	 
	 // echo ('filevideo: '.$filevideo.'<br />');
	 $videoArray = explode(" ",$filevideo); // break up video details into individual infobytes
	 foreach ($videoArray as $indvideoArray)
	 {
	 // echo ($indvideoArray." | ");
	 $whereisx = strpos($indvideoArray,'x');
	// echo $whereisx." | ";
	 	if ($whereisx>2) // check that x is from 640x480, not from 0x12345
	 	{
	 		$resolution=$indvideoArray;
	 		// echo ('<span color=red>aspect:'.$resolution.'</span>');
		}
	 }
	 //
	...
	$pathtoqueuedfile=$pathtoqueue.$tempfile2tobemoved;								// "/var/www/html/jonahv1/queue/" + corrected filename
	...
	else
	{
		...
		else 
		{
			...
			//rename($loc1,$loc2); 	  													// MOVE UPLOADED FILE TO QUEUE DIRECTORY if 
		...
		//						"tvp"		$client		$tempfile2tobemoved $videofilesize	$timestamp	$filevideo	$fileaudio	$duration
				...
				// $add2queue['category'] 	= "TVP"; // see below
				
				$add2queue['category']  = getorgbyDir($watchfolder);
	  ...
	  $codec = stristr($codec1," ",True); 				// everything before space
		...
		$add2queue['aspect'] = getAR($resolution);
		...
		//		below happens very quickly - still, is there scope to lock the file in case another script modifies it between reading, appending and writing?
				...
				if (!copy('/var/www/html/jonahv1/queue/queue.json','/var/www/html/jonahv1/queue/queue.bak')); 
				{
   				 	//echo ('failed to backup queue.json<br />');
					rename('/var/www/html/jonahv1/queue/queue.json','/var/www/html/jonahv1/queue/queue.bak');
				}
				file_put_contents('/var/www/html/jonahv1/queue/queue.json', $newQueue); 				// 	replace by mySql record update (add item to bottom of queue)
	...
	$testAudio = rationaliseAudio($fileaudio);
	$testVideo = rationaliseVideo($filevideo);
	$displayDur  = substr($duration,0,-3);
	$tmpOrg = getorgbyDir($watchfolder);
		
	// show upload of media file in a table row
	echo ("<tr>");																																		// cleanup
	echo ("<td class='td-name underlined'><span class='countdown'>".$client." ($tmpOrg)</span></td>");					// isolating video filename							// cleanup
	echo ("<td class='td-path underlined'><span class='countdown'>".$videofilename."</span></td>");																// cleanup
	echo ("<td class='td-size underlined'><span class='countdown'>".$videofilesize." Mb</span></td>"); 		// filesize 										// cleanup
	echo ("<td class='td-timestamp underlined'><span class='countdown'>".$videouploaded." GMT</span> ".$howlongago."</td>"); 	// time last modified GMT 		// cleanup
	echo ("<td class='td-duration underlined'><span class='countdown'>".$displayDur."</span></td>");
  ...
  echo ("<td class='td-video underlined'>".substr($filevideo,0,25)."<a href='#' class='tooltip' title='$filevideo'>...more</a></td>"); 												// ffmpeg output relevant to videofile 				// cleanup
	echo ("<td class='td-audio underlined'>".substr($testAudio,0,25)."<a href='#' class='tooltip' title='$testAudio'>...more</a></td>"); 												// ffmpeg output relevant to videofile 				// cleanup
	echo ("<td class='td-status underlined'><span class='countdown'>".$filestatus."</span></td>");							// show filestatus text
	echo ("<td class='td-icon underlined'><img class='ftpstatus' src='/jonahv1/images/".$filestatus2.".png' /></td>"); 		// show filestatus icon
	echo ("</tr>");
		...
		echo ("<tr><td><span class='countdown'>".$client."</span></td><td colspan='7'> ".$filefound." (not valid media type - ".$fileage." mins ago)</td><td><img src='/jonahv1/images/unknown.png' /></td></tr>");
		
			if ((preg_match("/\.".$allowed_xml_types."$/i",$filefound)) && ($fileage>$minstowait)) // check for allowed xml types that have been 5 mins
			{
				//rename($filefound,$pathtoxml.$loc3);				// move xml to '/var/www/html/jonahv1/xml/' // removed for demo purposes
			}
			else 
			{
				if ($fileage>$minstowait) 
				{
				//rename($filefound,$orphanpath.$loc3);		// move non media, non-xml to '/var/www/html/jonahv1/orphans/' // removed for demo purposes
				}
			} // end xml type
		}
	}
...
} // end for each subfolder

}	// end for each folder under home
?>