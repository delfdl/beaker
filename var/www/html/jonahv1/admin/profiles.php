...
<select><option value='tvp'>TVP</option></select>&nbsp;<input type='submit' value=' select preset ' class='softbutton' /></form> <!-- // jquery to load preset -->
</form>
<br /><br />
...
<?php
	$fullPreset=file_get_contents('/var/www/html/jonahv1/profiles/tvp-preset.json'); 				// 	replace by mySql record update (add item to bottom of queue)
...
?>
<form action='' name='line<?php  echo ($i); ?>' id='line<?php  echo ($i); ?>'>
<tr>
<td>&nbsp;<?php  echo ($i); ?>&nbsp;</td>
<td><select class='prinput' name='-c:v'>
<?php  echo ("<option class='selected' value='".$preset['-c:v']."' selected>".$preset['-c:v']."</option>");
?>
<option value='libx264'>libx264</option><option value='none'>none</option></select></td>
<td><select class='prinput' name='-r'>
<?php  echo ("<option class='selected value='".$preset['-r']."' selected>".$preset['-r']."</option>");
?>
...
<td><input size='6' class='prinput' name='-b:v' value='<?php  echo ($preset['-b:v']); ?>'></input></td>
<td><input size='10' class='prinput' name='-s' value='<?php  echo ($preset['-s']); ?>'></input></td>
<td><input size='6' class='prinput' name='-g' value='<?php  echo ($preset['-g']); ?>'></input></td>
<td><input class='prinput-long' type='text' name='addition1' value='<?php  echo ($preset['addition1']); ?>' /></td>
<td><select class='prinput' name='-c:a'>
<?php  echo ("<option class='selected' value='".$preset['-c:a']."' selected>".$preset['-c:a']."</option>");
?>
...
<?php  echo ("<option class='selected' value='".$preset['-ac']."' selected>".$preset['-ac']."</option>");
?>
...
<?php  echo ("<option class='selected' value='".$preset['-ar']."' selected>".$preset['-ar']."</option>");
?>
...
<td><input size='6' class='prinput' value='<?php  echo ($preset['-b:a']); ?>' name='-b:a'></input></td>
<td><input class='prinput-med' type='text' name='addition2' value='<?php  echo ($preset['addition2']); ?>' /></td>
<td><select class='prinput' name='container'>
<?php  echo ("<option class='selected' value='".$preset['container']."' selected>".$preset['container']."</option>");
?>
<option value='mp4'>mp4</option><option value='aac'>aac</option><option value='f4v'>f4v</option></select></td>
<td>1</td>
<td><input class='prinput' type='checkbox' name='hls' value='<?php  echo ($preset['2pass']); ?>'<?php  if ($preset['2pass']) {echo " checked='yes'";} ?> /></td>
<td><input class='prinput' type='checkbox' name='hds' value='<?php  echo ($preset['2pass']); ?>'<?php  if ($preset['2pass']) {echo " checked='yes'";} ?> /></td>
<td><input class='prinput' type='checkbox' name='android' value='<?php  echo ($preset['2pass']); ?>'<?php  if ($preset['2pass']) {echo " checked='yes'";} ?> /></td>
</tr>
</form>

<?php  } // end if !presetPass
	 } // end for each
...
<?php  // include ('/var/www/html/jonahv1/inc/queue-view.php'); ?>
