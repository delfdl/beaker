...
function ffmpegExt($ffmpegCall, $logFile, $i) 
    ...
    $fullremote = "php -f /var/www/html/api/ffmpegcall.php -- -a ".$logFile." -b ".$i." -c ".$ffmpegCall." > /dev/null 2>/dev/null &" ;  // make async call to ffmpeg controller (not ffmpeg itself)
...
function ffmpegExtSynch($ffmpegCall, $logFile, $ffmpegThumb) 
    ...
    $fullremote = "php -f /var/www/html/api/ffmpegcallsync.php -- -a ".$logFile." -b ".$ffmpegThumb." -c ".$ffmpegCall." > /dev/null 2>/dev/null &" ;  // make async call to ffmpeg controller (not ffmpeg itself)
...
function transcodeSubtitle($preset,$srcfile,$org,$channel,$hqflag,$logFile,$tmpname,$i)
    ...
    $ffprobe_short = getProbeinfo($srcfile);
    $detailsJson = json_decode($ffprobe_short,true);
    $howmanySubs = $detailsJson['subtitlecount'];
    transcodeLog($logFile,'errr, about to figure out extraction of subs '.$howmanySubs); 
    $txtlang = '';
 
    $fireandforget = "> /dev/null 2>/dev/null &" ; // add to exec (ffmpeg) call to make asynchronous (mmmm!)
      // $fireandforget = '';
      
    $outputdir = '/home/secure/subs/'.$tmpname.'/';  
      
         if(!file_exists($outputdir))    
        {
            mkdir($outputdir, 0777, true);
            transcodeLog($logFile,'creating subs path - '.$outputdir); 
        }   

    
    // work out $trackno
    // work out $txtlang; 
    $outputPath = $outputdir.'subs_'.$txtlang.'.srt';
    

    // ffmpegOptions = "ffmpeg -i '".$srcfile."' -map 0:".$trackno." -an -vn -codec:s srt -y '".$outputPath."' '.$fireandforget; 
    }    
...
function getlogTime($piece)
    {
    // asterix * added specifically for this extraction
        // echo ($piece);
        $fred = explode('*',$piece);
        if (isset($fred[1]))
                {     
...
$nameToCode3 = array(
   ...
   // echo "iso6391: ".$iso6391."<br />";
    ...
    else 
    {
        ...
        {
            if (substr($key,0,strlen($iso6391))==$iso6391)
            {
                $iso6392 = $wrongCode;
                $alt=1;
            }
