...
// s/w transcode using php
     ...
     $mi = getMediaInfo($srcfile);
       ...
       //  }
        ...
        $subtitlecount = $tmpJson['subtitlecount'];
        ...
        // check resolution (SD or HD) to determine number of video profiles
         ...
         if (intval($actualWidth)<1) 
         {
             $actualWidth = $tmpJson['width'];
         }
     ...
     {
        $presettoUse = '/home/presets/'.$org.$preset.'.preset';
        if (!file_exists($presettoUse))
        {
          $presettoUse = '/home/presets/'.$org.$encRes.'.preset';
          transcodeLog($logFile,'nominated '.$preset.' preset not found, reverting to '.$encRes);  
        } else
        {
          transcodeLog($logFile,'using nominated '.$preset.' preset'); 
        }
