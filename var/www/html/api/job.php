...
// receive job parameters (src, tmpname, org, optional hqflag, option debug, option rewrap, option preset)
      ...
      if (isset($_GET["preset"])) {$preset = $_GET["preset"];} 
        else {$preset = 'default';}                                            // force encoding to use specific preset
...
// create logfile using job reference
    $path = '/var/www/html/api/jobs/'.$org.'/'; // should already exist
    // mkdir($path, 0755);
    $logFile = $path.$uniqueRef.'.log';
    $handle = fopen($logFile, 'w');
    $newtime = date('d/m/y H.i:s');
    $playbackchunk = $org.'/'.$channel.'/'.$tmpname.'/';
    $data = '['.$newtime.'] '.$responseJson['response'].' by job.php -'.$src.' *'.microtime().'* to /mount/wing/vod/'.$playbackchunk.' (|http://vod1.ot2.tv/vod/'.$playbackchunk.$tmpname.'.ism/'.$tmpname.'.f4m|'.$playbackchunk.'|)';
    fwrite($handle, $data);
    fclose($handle);
    chmod($logFile, 0777); 
