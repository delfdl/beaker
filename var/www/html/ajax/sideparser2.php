...
// $loopDesc  = array('White Tigers','African Lions','African Wild Dogs','Meerkats','African Mixed pole','African Mixed hut',''); 
$loopArray = array('aa-port1','aa-port2','aa-port3','aa-port5','aa-port6','aa-port7','aa-port1-monitor','aa-port2-monitor','aa-port3-monitor','aa-port5-monitor','aa-port6-monitor','aa-port7-monitor');
$loop = 0;
$streamCheck = array();
foreach ($loopArray as $keyid) {$streamCheck[$keyid]=0;}
//$streamCheck['aa-port1']=0;$streamCheck['aa-port2']=0;$streamCheck['aa-port3']=0;$streamCheck['aa-port5']=0;$streamCheck['aa-port6']=0;$streamCheck['aa-port7']=0;
//$streamCheck['aa-port1-monitor']=0;$streamCheck['aa-port2-monitor']=0;$streamCheck['aa-port3-monitor']=0;$streamCheck['aa-port5-monitor']=0;$streamCheck['aa-port6-monitor']=0;$streamCheck['aa-port7-monitor']=0;
...
foreach($chunks as $chunk)
      {
        ...
        foreach ($loopArray as $loopMatch)
        {
        	if ($nodename==$loopMatch) // if either aa-port1 or aa-port1-monitor 
        	{
        		$streamCheck[$nodename] = $bw_in; // use incoming bandwidth value
        	}
        }
      }
echo('<div id="general">'); 

// ok, the below code is shit, should really be in a loop using an array, but I was in a rush. I will [sometime] revisit and tidy up

echo ('<div id="dyn" class="hiddenpars"><table cellpadding="0" width="100%" cellspacing="0" border="0" class="dTable" id="dynamic">');
echo('<thead><tr><th>Camera Enclosure </th><th>SDI </th><th>CC </th><th>HD </th></tr></thead>');

if ($streamCheck['aa-port1']!==0)         {$monState1='active';} else {$monState1='idle';}
if ($streamCheck['aa-port1-monitor']!==0) {$monState2='active';} else {$monState2='idle';}
echo ('<tr style="background-color:#fefefe;"><td>&nbsp;AA-1 White Tigers &nbsp; </td><td>&nbsp;1</td><td><img src="/images/'.$monState2.'.gif" title="'.round($streamCheck["aa-port1-monitor"]/1024,2).' kb/s" align="absmiddle" style="vertical-align:middle;" /></td><td><img src="/images/'.$monState1.'.gif" title="'.round($streamCheck["aa-port1"]/1048576,2).' mb/s" align="absmiddle" style="vertical-align:middle;" /></td></tr>');

if ($streamCheck['aa-port2']!==0) {$monState1='active';} else {$monState1='idle';}
if ($streamCheck['aa-port2-monitor']!==0) {$monState2='active';} else {$monState2='idle';}
echo ('<tr style="background-color:#f0f0f0;"><td>&nbsp;AA-2 African Lions &nbsp; </td><td>&nbsp;2</td><td><img src="/images/'.$monState2.'.gif" title="'.round($streamCheck["aa-port2-monitor"]/1024,2).' kb/s" align="absmiddle" style="vertical-align:middle;" /></td><td><img src="/images/'.$monState1.'.gif" title="'.round($streamCheck["aa-port2"]/1048576,2).' mb/s" align="absmiddle" style="vertical-align:middle;" /></td></tr>');

if ($streamCheck['aa-port3']!==0) {$monState1='active';} else {$monState1='idle';}
if ($streamCheck['aa-port3-monitor']!==0) {$monState2='active';} else {$monState2='idle';}
echo ('<tr style="background-color:#fefefe;"><td>&nbsp;AA-3 African Wild Dogs &nbsp; </td><td>&nbsp;3</td><td><img src="/images/'.$monState2.'.gif" title="'.round($streamCheck["aa-port3-monitor"]/1024,2).' kb/s" align="absmiddle" style="vertical-align:middle;" /></td><td><img src="/images/'.$monState1.'.gif" title="'.round($streamCheck["aa-port3"]/1048576,2).' mb/s" align="absmiddle" style="vertical-align:middle;" /></td></tr>');

if ($streamCheck['aa-port5']!==0) {$monState1='active';} else {$monState1='idle';}
if ($streamCheck['aa-port5-monitor']!==0) {$monState2='active';} else {$monState2='idle';}
echo ('<tr style="background-color:#f0f0f0;"><td>&nbsp;AA-5 Meerkats &nbsp; </td><td>&nbsp;5</td><td><img src="/images/'.$monState2.'.gif" title="'.round($streamCheck["aa-port5-monitor"]/1024,2).' kb/s" align="absmiddle" style="vertical-align:middle;" /></td><td><img src="/images/'.$monState1.'.gif" title="'.round($streamCheck["aa-port5"]/1048576,2).' mb/s" align="absmiddle" style="vertical-align:middle;" /></td></tr>');

if ($streamCheck['aa-port6']!==0) {$monState1='active';} else {$monState1='idle';}
if ($streamCheck['aa-port6-monitor']!==0) {$monState2='active';} else {$monState2='idle';}
echo ('<tr style="background-color:#fefefe;"><td>&nbsp;AA-6 African Mixed Pole &nbsp; </td><td>&nbsp;6</td><td><img src="/images/'.$monState2.'.gif" title="'.round($streamCheck["aa-port6-monitor"]/1024,2).' kb/s" align="absmiddle" style="vertical-align:middle;" /></td><td><img src="/images/'.$monState1.'.gif" title="'.round($streamCheck["aa-port6"]/1048576,2).' mb/s" align="absmiddle" style="vertical-align:middle;" /></td></tr>');

if ($streamCheck['aa-port7']!==0) {$monState1='active';} else {$monState1='idle';}
if ($streamCheck['aa-port7-monitor']!==0) {$monState2='active';} else {$monState2='idle';}
echo ('<tr style="background-color:#f0f0f0;"><td>&nbsp;AA-7 African Mixed Hut &nbsp; </td><td>&nbsp;7</td><td><img src="/images/'.$monState2.'.gif" title="'.round($streamCheck["aa-port7-monitor"]/1024,2).' kb/s" align="absmiddle" style="vertical-align:middle;" /></td><td><img src="/images/'.$monState1.'.gif" title="'.round($streamCheck["aa-port7"]/1048576,2).' mb/s" align="absmiddle" style="vertical-align:middle;" /></td></tr>');

echo ('</table></div>');
 
echo('</div>');

// save local stream data for 
  $localBeaker = $localpath.'test/data/localbeaker.json';
	$channelString = json_encode($streamCheck);
	file_put_contents($localBeaker, $channelString); // saving local bandwidth
	
	
       
      
echo ('<div class="divider"><span></span></div>');      
echo ('<div id="somestuff" style="padding:3px;">Bitrate variance this hour<br />');     
echo ('Min: <span style="font-size:12pt; font-weight:bold;">'.$localstatsJson['min'].'</span> mb/s | ');
echo ('Max: <span style="font-size:12pt; font-weight:bold;">'.$localstatsJson['max'].'</span> mb/s <br />');
echo ('</div>');
